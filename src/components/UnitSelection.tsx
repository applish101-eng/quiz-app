...
export default function UnitSelection({ subject, onSelect, onBack }: UnitSelectionProps) {
  const filteredUnits = units.filter(
    (u) => (u.subject ?? 'operationalResearch') === subject.name
  )
      ...
      <div className="space-y-3">
        {filteredUnits.map((unit) => {
          const count = quizData.filter(
            (q) => q.subject === subject.name && q.unit === unit.number
          ).length

          return (
            <button
              key={`${subject.name}-${unit.number}`}
              onClick={() => {
                ...
                <div className="flex items-center gap-4 p-4 sm:p-6">
                  <div
                    className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full text-lg font-bold text-white ${
                      count === 0 ? 'bg-muted-foreground/30' : 'bg-emerald-500'
                    }`}
                  >
                    {unit.number}
                  </div>
